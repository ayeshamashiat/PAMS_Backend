...
const getStudentProgress = async (req, res) => {
  try {
    ...
    const isEligible = cgpa >= 2.5 && earnedCredits >= 9 && hasSupervisor;

    const currentProposal = await ThesisProposal.findOne({
    ...
    if (!isEligible) {
      const reasons = [];
      if (cgpa < 2.5) reasons.push("CGPA below 2.5");
      if (earnedCredits < 9) reasons.push("Insufficient credits (< 9)");
    ...
    res.json({
      ...
      proposal: currentProposal
        ? { status: currentProposal.status, hasProposal: true }
        : { hasProposal: false },
    });
  } catch (e) {
...
const getMyProposal = async (req, res) => {
  try {
    ...
    console.error("Error fetching student proposal:", e);
    res.status(500).json({ message: "Server error", error: e.message });
  }
};
...
const downloadProposalPDF = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="thesis-proposal-${proposal._id}.pdf"`
    );
    fs.createReadStream(filePath).pipe(res);
  } catch (e) {
    console.error("Error downloading proposal PDF:", e);
    res.status(500).json({ message: "Server error", error: e.message });
  }
};
  ...
  try {
    ...
    if (!student.supervisor_id) {
      return res.status(400).json({
        message:
          "You must have a supervisor assigned before submitting a proposal",
      });
    }
    ...
    const cgpa = Number(student.cgpa || 0);
    const isEligible =
      earnedCredits >= 9 && cgpa >= 2.5 && !!student.supervisor_id;

    if (!isEligible) {
      return res.status(400).json({
        message:
          "You are not eligible to submit a thesis proposal. Please ensure you meet all requirements.",
      });
    }

    // Existing proposal?
    const existingProposal = await ThesisProposal.findOne({
      student_id: student._id,
    });
    ...
    if (!canSubmit) {
      return res.status(400).json({
        message:
          "You already have a proposal submitted or approved. You can only resubmit if it was rejected or revision was requested.",
      });
    }
    ...
    if (
      ...
      if (existingProposal.feedback) {
        if (!existingProposal.feedbackHistory)
          existingProposal.feedbackHistory = [];
...
const checkSupervisorEligibility = async (req, res) => {
  try {
    ...
    if (!student) {
      return res
        .status(404)
        .json({ isEligible: false, message: "Student not found" });
    }
...
const checkAssignmentStatus = async (req, res) => {
  try {
    ...
    if (!assignment) {
      return res
        .status(404)
        .json({ message: "No supervisor assignment found for this student." });
    }
